...
M3 仓储库存数字化管理已由 Owner 授权新开，与 M1-FIX 并列。当前轮次为 M3-R6（IN_PROGRESS，已开工）：入库拍照识别服务——**照片不出内网，走本地部署**，暂部署在这台 Mac；服务端骨架、约束校验层、Frappe 侧入口与人工校对界面均已交付，**真实跨进程端到端已验证通过**；**真实样本准确率与耗时均已实测达标**（50 张 / 250 字段；选用 PaddleOCR `small` 档：批号 100% / 代码 98% / 生产日期 94% / 效期 90% / 品名 92%，全字段 82%，**单张均 8.1 s（7~9 s）**，accuracy 与 ≤10 s 耗时标准均达标；换档免装依赖）；**分档实测（2026-09-21）**：大字/5 个结构化字段全字段 `small` 82% / `medium` 86%；小字（16 个模板恒定串 × 50 张）整串命中 `small` **83.5%** / `medium` 85.2%，**串越长字越小命中越低**（运输注意事项整句仅 25/50）；**手写档给不出准确率**——无真值来源，人工转写在可获分辨率下同样辨认不出，姓名 50 张无一读出；换 `medium` 每提 1 个点要付 4 倍耗时，**维持 `small`**；并已交付「**入库提交后自动生成待检证 / 货位卡**」，并修完 M3-R4/R5 的**打印纸型缺陷**（原先手动打印全按 A4 出）与**货位二维码小标签**（全量 224 个标签验证；⚠ 当时只验了**页数与纸型**、没验字形——2026-09-23 发现容器**无中文字体**，四个打印格式的汉字其实**全被丢弃**，已修，见 R6 主文档第九之十一节）；**物料建档守卫**已从「存不存在」加固为「能不能用」（未停用 / 有库存 / **有批号**）；并让拍照识别页可**手工补录 OCR 读不到的字段**（储存条件 / 生产车间 / 效期类型 / 供货单位 / 生产单位 / 原厂批号 / 包装构成），全部流进待检证与货位卡；M3-R5 已执行完毕（REVIEWING）：货位二维码标签与手机扫码页（展示全部字段、不脱敏）；M3-R4 已实现待检证与自产/外购货位卡自动生成；M3-R3 已交付出库放行门禁、货位变更、`效期预警` 与 `库级盘点三对账` 报表、`仓库工作台` 入口；M3-R2 已落地 `hb_inventory_app`、主数据与台账报表并把货位树改挂到 `3904`；M3-R0、M3-R1 亦为 REVIEWING。
...
- `docs/inventory/仓库工作台操作手册.md` —— 从哪进、左侧各条目用途、入库/出库/移库怎么开单、
  哪些字段必填、常见报错怎么处理
...
- M3-R6 入库拍照识别服务**已开工**（IN_PROGRESS）：服务骨架 `services/hbos_ocr/`、约束校验层、Frappe 侧入口与人工校对界面均已交付并验证（约束层单测 42 条 + C2 抽取规则单测 37 条，共 **79** 条全通过）；后端可插拔（`stub`/`c1_local_vlm`/`c2_ocr`）且缺依赖不崩；门禁已确认：**照片不出内网 → 走本地部署**，暂部署在这台 Mac（`host.docker.internal` 实测可达，服务原生跑在 Mac 上，不塞进 Frappe 容器）；识别范围收敛为 5 个字段；人工校对强制、结果不直接入账；**真实样本准确率与耗时均已实测达标**（50 张 / 250 字段；`small` 档全字段 82%、**单张均 8.1 s（7~9 s）**，accuracy 与 ≤10 s 均达标；换档免装依赖，只是换模型名；**分档实测见 R6 主文档第九之五·二节**：大字全字段 `small` 82% / `medium` 86%，小字整串命中 `small` **83.5%** / `medium` 85.2%（长串最弱，运输注意事项整句仅 25/50），**手写档无真值、给不出准确率**——姓名 50 张无一读出，`操作人/日期` 标签后为空 34/50）；因 C2 不输出逐字段置信度，"高置信度错误 ≤1% / 低置信度召回 ≥90%" 两项**仍无法测量**；**识别范围只做自产**（外购到货时已有现成待检证与货位卡，不走本流程）；待 Owner 就置信度那条决策；**入库单提交后自动生成「待检证+货位卡」合并 PDF 并挂到批次附件**（新建 `doc_gen.py` 挂 `Stock Entry.on_submit`；`Batch` 表单加「重新生成」按钮；实测 2 批次 1.55s，失败不阻断入库）
...
- M3-R4 待检证与货位卡自动生成已执行完毕并进入 REVIEWING：新增三个 Print Format（`HBOS 待检证` 75×110mm 小标签、`HBOS 自产货位卡`、`HBOS 外购货位卡`，后两者 A4），挂在 `Batch` 上按批次数据生成（**当时只支持手动点打印**；「入库后自动生成」在 M3-R6 第五批补齐，且其纸型缺陷亦于该批修复）；新增打印辅助方法与 `Batch.hbos_source_type` 字段；定案「多货位在货位号单元格内逐行列出」；件数按容器类型汇总；流水表按单据净减少判断（库内移库不计入发出）
- M3-R3 出库核销、货位变更、效期预警与盘点导出已执行完毕并进入 REVIEWING：新增出库放行门禁（`before_submit`，限 `Delivery Note` 与 `Stock Entry` Material Issue；待检批次出库被拦截、已放行批次通过、入库与移库不受影响）；货位变更复用原生 Material Transfer 并复验通过；新增 `效期预警`（紧急度分级）与 `库级盘点三对账`（ERP数量 + 留空的货位卡/实物列）两个报表；补建 `仓库工作台` 入口（补 M3-R2 遗漏）
...
13. M3-R3 已执行完毕（`docs/milestones/M3_R3_出库核销效期预警与盘点导出.md`）：出库放行门禁、货位变更、效期预警与库级盘点三对账报表、仓库工作台入口。前置口径：出库批次选取原生即 FIFO；负库存采用默认「不允许」。当前 REVIEWING。
