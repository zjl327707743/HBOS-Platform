...
当前 M0 工程启动阶段已完成并封板，M0-REMOTE 已完成，M1 已完成 M1-R0 规划收口、M1-R1 对象模型验证收口和 M1-R2 配置试运行方案收口。M1-R3 已通过 Codex 审查，但实际结果为 PARTIAL / BLOCKED，现收口为 BLOCKED：仅部分 TEST 基础数据落库，Employee / Shift / Checkin / Attendance 闭环因运行态 ORM 写入阻塞未完成。
...
- M1-R3A 运行态阻断诊断与 TEST 数据隔离 / 清理方案为 PLANNED，尚未启动
