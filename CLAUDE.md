...
当前并行里程碑：M1-FIX（考勤功能补漏，IN_PROGRESS）与 M3（仓储库存数字化管理，IN_PROGRESS，M3-R0 至 R5 已执行、M3-R6 方案已就绪）。当前阶段门禁文档为 `docs/milestones/M1_START_GATE.md` 与 `docs/milestones/M3_START_GATE.md`。
...
- 未经用户逐轮授权，不启动 M3-R6 执行（方案已就绪，需 Owner 确认门禁后授权）及之后轮次，不创建 M3 相关 DocType，不写 M3 业务代码，不创建库存业务数据
- 不写 Docker Compose（`hb_inventory_app` 的挂载已由 Owner 授权补入，不得擅自再改）
- 不做前端驾驶舱
- 不接飞书真实写入
- 不修改 Frappe/ERPNext/HRMS 核心源码
- 不提交 `.env`、密钥、token、真实业务数据、Excel/CSV
...
- 当前阶段门禁文档必须检查：M1 阶段为 `docs/milestones/M1_START_GATE.md`，M3 阶段为 `docs/milestones/M3_START_GATE.md`（已创建），未来 M2 阶段为 `docs/milestones/M2_START_GATE.md`。
