...
当前并行里程碑：M1-FIX（考勤功能补漏，IN_PROGRESS）与 M3（仓储库存数字化管理，IN_PROGRESS，M3-R0 / R1 / R2 已执行）。当前阶段门禁文档为 `docs/milestones/M1_START_GATE.md` 与 `docs/milestones/M3_START_GATE.md`。
...
- 未经用户逐轮授权，不启动 M3-R3 及之后轮次，不创建 M3 相关 DocType，不写 M3 业务代码，不创建库存业务数据
