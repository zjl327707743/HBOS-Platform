...
def _require_hr_write():
    """Manual API guard; scheduler runs as Administrator and uses the same service safely."""
    if frappe.session.user == "Administrator":
        return
    roles = set(frappe.get_roles())
    if not roles.intersection({"HR Manager", "System Manager"}):
        raise frappe.PermissionError("你无权触发调休外部同步。")
    ...
    for row in pending:
        ...
        try:
            ...
            prompt = build_overtime_prompt(
                ...
                row.remarks or "", year)
            try:
                text = call_llm(cfg, prompt, contains_pii=True)
            except Exception as e:
