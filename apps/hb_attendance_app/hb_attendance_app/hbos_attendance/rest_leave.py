...
_PROMPT_HEAD = (
    ...
    "2. 若说明未出现任何姓名，视为本人；\n"
    "3. 说明原文的日期常见写法有「8月2号」「8月2日」「08.02」「8/2」等；\n"
    "4. 年份以【申请年】为准；\n"
    "5. 找不到任何日期就只输出「无」；\n"
...
# 中文带年份：2026年8月2日 / 2026年8月2号（自带年份）
_CN_FULL_DATE_RE = re.compile(r"(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*[日号]")
# 中文式：8月2日 / 8月2号（年份取【申请年】）
# 实测 104 条真实说明里「月+号」71 条、「月+日」13 条，号是主要写法，两者必须都认。
_CN_DATE_RE = re.compile(r"(\d{1,2})\s*月\s*(\d{1,2})\s*[日号]")
# 短式：08.02 / 8/2（年份取【申请年】）；前后不能再接数字或分隔符，
# 避免把完整日期的片段（26.08）或版本号之类再解读一次
_SHORT_DATE_RE = re.compile(r"(?<![\d./-])(\d{1,2})\s*[./]\s*(\d{1,2})(?![\d./-])")

# 自带年份的写法（第一遍扫描用）：这些不依赖【申请年】
_YEARED_DATE_RES = (_FULL_DATE_RE, _CN_FULL_DATE_RE)
...
def parse_overtime_dates(text, year):
    ...
    接受四种写法：完整日期（2026-08-02 / 2026.8.2 / 2026/8/2 / 2026年8月2号）、
    中文式（8月2号 / 8月2日）、短式（08.02 / 8/2，年份取【申请年】）。
    每个候选都按真实日历校验，非法日期一律不产出，而不是原样当日期返回。
    年份缺失时不猜年份：只保留文本里自带年份的完整日期。

    宁可多提不可漏提：多出来的候选只会让核实更严格（核实不通过 → 转人工），
    漏掉的加班日却会把真实加班判成「解析失败」，所以范围/时间描述里的数字
    若构成合法日期一律提取，不做「看起来像时间就跳过」的启发式。
    """
    if not text:
        return []
    text = str(text)
    ...
    # 第一遍：自带年份的日期。命中的区间先屏蔽，
    # 免得 "2026.08.02" 又被下面的短式规则按 "26.08" 二次解读，
    # 也免得 "2026年8月2号" 再被中文式用【申请年】重新解读一遍（年份会打架）。
    chars = list(text)
    for pattern in _YEARED_DATE_RES:
