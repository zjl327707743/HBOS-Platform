...
M1-R5 已获用户授权执行并进入 REVIEWING。后续 R6/R7 仍为 **PLANNED**，需用户逐轮明确授权后方可启动。不得在未经授权的情况下启动任何后续轮次。

### 5.1 M1-R5：HRMS 配置基线、考勤工作台与月度汇总 Demo

**状态**：REVIEWING（主文档已交付，等待审查或验收）。
...
| M1-R5 | REVIEWING | 主文档已交付，等待审查或验收 |
| M1-R6 | PLANNED | 待用户授权启动 |
| M1-R7 | PLANNED | 待用户授权启动 |
| M1-R8 | PLANNED | 可选缓冲轮，不预先承诺一定执行 |

M1-R5 已进入 REVIEWING。M1-R6/R7 均为 PLANNED，未启动。不得在未经用户逐轮授权的情况下启动后续轮次。
