...
- `docs/milestones/M3_R2_入库登记与批次货位台账方案.md`：M3-R2 入库登记与批次货位台账方案。
...
| M3-R2 | 入库登记与"产品—批次—货位"台账 | PLANNED（方案已就绪） |
...
7. M3 仓储库存数字化管理已由 Owner 授权新开，与 M1-FIX 并列；M3-R0 只读盘点与需求确认、M3-R1 货位主数据建模与粒度验证均已交付并进入 REVIEWING。M3-R1 已建立 203 个货位的 `Warehouse` 树，并通过"按批号查货位"双向验证；同时更正了 M3-R0 关于批次存储位置的结论（v16 中批次在 `Serial and Batch Entry`，不在 `SLE.batch_no`）。M3-R2 方案已交付（四项设计决策已确认，含授权新建 `hb_inventory_app`），当前 PLANNED / 待 Owner 授权执行。
