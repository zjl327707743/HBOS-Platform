...
- M1-R3E 标记为 `REVIEWING`，已形成配置复核清单与业务口径确认表，等待 Codex 审查。
- M1-R4 未启动。

M1-R3D 已聚焦 HRMS 原生考勤异常口径与配置 Gap 诊断，覆盖迟到 / 早退标记、缺卡异常、全天缺勤生成、Leave Allocation 请假前置、加班和节假日出勤报表口径，全部归入 HRMS 配置、原生 / 自定义报表、海滨业务规则定义和未来自定义 App 候选四类。M1-R3E 已形成配置复核清单与业务口径确认表，仍不得接真实考勤机、不得接真实飞书、不得录入真实生产数据、不得创建自定义 App、不得开发业务代码。
