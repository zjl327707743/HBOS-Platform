...
建议 M1-R3D 聚焦 HRMS 原生考勤异常口径与配置 Gap 诊断，至少覆盖迟到 / 早退标记、缺卡异常、全天缺勤生成、Leave Allocation 请假前置、加班和节假日出勤报表口径。M1-R3D 仍不得接真实考勤机、不得接真实飞书、不得录入真实生产数据、不得创建自定义 App、不得开发业务代码。
