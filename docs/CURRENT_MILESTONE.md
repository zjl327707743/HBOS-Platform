...
M3-R3 已执行完毕（REVIEWING）：新增出库放行门禁（`before_submit`，限 `Delivery Note` 与 `Stock Entry` Material Issue；待检批次出库被拦截、已放行批次通过、入库与移库不受影响）；货位变更复用原生 Material Transfer 并复验通过；新增 `效期预警` 与 `库级盘点三对账` 两个报表；补建 `仓库工作台` 入口（补 M3-R2 遗漏；**该入口两个缺陷已于 2026-09-23 修复**——顶层图标曾误用与 ERPNext 原生 `Stock` 相同的 `stock` 致点错进原生模块且回不来，以及**桌面图标因 label 与侧边栏名不一致被 Frappe 静默丢弃**（父图标被丢弃还会连带子图标），现已三者同名，详见其主文档 6.2 节）。**2026-09-23 同日再次收敛**：Owner 问「为什么要单独做仓库工作台、不能集成在原生库存里」，结论为**保留但瘦身**——侧边栏由 13 条减到 6 条，撤掉与原生库存重复的 7 条；并在 `Stock Entry` / `Batch` 两个原生表单补「返回仓库工作台」出口。**同日晚些时候该「撤掉」被纠正**：Owner 提供截图显示进单据后**左侧边栏整条换成原生「库存」**、面包屑第一项也是「库存」——那几条其实是**承重的**（框架的 `resolve_sidebar` 规则 1「当前侧边栏已链接该单据则保持不变」，以及 `Workspace Shortcut` 决定的面包屑归属）。已加回 4 条原生入口（Stock Entry 只留一条）+ 3 条快捷方式，`Item` 因 `Home` 已有快捷方式且查询无 `order_by` 而**有意跳过**。详见 `M3_R6` 主文档第九之十六、九之十七节。前置口径：出库批次选取原生即 FIFO；负库存采用 ERPNext 默认「不允许」。
