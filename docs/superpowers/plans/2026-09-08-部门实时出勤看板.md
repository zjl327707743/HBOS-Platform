...
在测试文件顶部 import 区追加 `from pathlib import Path`，并在模块级加：
...
> 说明：`live_sync` 与 workspace 入口分别在 Task 4 / Task 5 落地，其契约断言在对应任务追加，本任务不涉及。
...
在测试文件模块级（`DATA_PY` 之后）追加 `WS_JSON` 路径常量，并新增测试类：
