...
- 恒等关系必须成立（每个 state 恰好归入一处，逐部门与全厂）：
  `expected == present + noCard + outOnly + unknownTime + notStarted + absent`。
- 「未打卡」`noCard` 只含**班次已开始却无到岗卡**（absent_expected / no_pair / pending）= 真预警；
  `out_only`（仅下班卡）、`fact_none`（班次未定）、`before_start`（未开始）**各自独立成桶**，
  **一律不计入未打卡**——否则 9 点会把夜班收工与班次未定者误报为未到（实测 176 → 14）。
...
- Produces（后续任务依赖，签名锁定）：
  ...
  - `summarize_rows(rows: list[dict]) -> dict` — 返回
    `{"total","expected","present","late","noCard","outOnly","unknownTime","notStarted","absent","leave","rest","exempt","attendance_rate"}`
  ...
  - `dept_summary(rows: list[dict]) -> list[dict]` — 每项
    `{"dept","total","expected","present","noCard","outOnly","unknownTime","notStarted","late","absent","leave"}`，
...
class SummarizeRowsTest(unittest.TestCase):
    ...
    def test_absent_is_separate_from_no_card(self):
        s = summarize_rows([row("absent_day"), row("fact_none")])
    ...
    def test_identity_holds_exactly(self):
        ...
        self.assertEqual(s["notStarted"], 1)
        self.assertEqual(s["absent"], 1)
        self.assertEqual(s["expected"], 12)
...
（`summarize_rows([])` 已含全部桶键，无需额外初始化函数。）
...
`kpiHtml` 改为接服务端 stats，并在下方补一行「其他桶」说明（不混入未打卡，数字可加和）：
...
在 `hooks.py` 的 `scheduler_events["cron"]` 中新增一项（**只一条**）：
...
> 2026-09-11 实测修正：Frappe cron 按 **系统时区** 判定，本环境为 `Asia/Shanghai`，
> 故 `0 9` 即北京 09:00（容器 OS 的 UTC 不影响）。原计划挂 `0 1`+`0 9` 两条是错的——
> `Scheduled Job Type` 以 `method` 为唯一键，同一方法挂多条会互相覆盖，
> 可能被覆盖成非 9 点而导致守卫静默拒发。
