...
- **口径零改动**：异常判定必须复用 `board_stats` 的 `is_late` / `PRESENT_STATES` / `NO_CARD_STATES` 等既有常量，**不得重写判定规则**。
...
**Interfaces:**
- Consumes: `board_stats.is_late(r)`、`board_stats.PRESENT_STATES`、`board_stats.ABSENT_STATES`、`board_stats.OUT_ONLY_STATES`、`board_stats.UNKNOWN_TIME_STATES`、`board_stats.NOT_STARTED_STATES`、`board_stats.NO_CARD_STATES`、`board_stats.dept_summary(rows)`；`get_data()["rows"]` 的行结构（`dept / num / name / state / kind / tags / anomaly_hidden`）
...
def collect_exceptions(rows):
    ...
    判定逻辑复用 board_stats 的常量与谓词（不在此重写，避免第四套口径），
    骨架严格照搬 summarize_rows 的 if/elif 链。
    返回只含至少一项异常的部门，部门顺序与 dept_summary 一致。
    """
    ...
    for r in rows:
        ...
        buckets.setdefault(dept, {"dept": dept, "noCard": [], "late": []})
        if r.get("anomaly_hidden"):
            continue
        st = r.get("state")
        if st in ("exempt", "rest", "leave", "unknown"):
            continue
        if r.get("kind") != "shift":
            continue
        person = {"name": r.get("name") or "", "num": r.get("num") or ""}
        if bs.is_late(r):
            buckets[dept]["late"].append(person)
        if st in bs.PRESENT_STATES:
            pass                      # 到岗（迟到者已单独记入 late）
        elif st in bs.ABSENT_STATES:
            pass                      # 缺勤：仅回顾模式的 HRMS Absent，本轮不列
        elif st in bs.OUT_ONLY_STATES:
            pass                      # 跨天夜班只刷到下班机，不算真预警
        elif st in bs.UNKNOWN_TIME_STATES:
            pass                      # 班次未定：系统不知其上班时间
        elif st in bs.NOT_STARTED_STATES:
            pass                      # 未到上班时间
        elif st in bs.NO_CARD_STATES:
            buckets[dept]["noCard"].append(person)
        else:
            buckets[dept]["noCard"].append(person)   # 与 summarize_rows 的兜底一致

    # 部门顺序对齐 dept_summary（未打卡+迟到降序，其次在册人数降序）
    order = {s["dept"]: i for i, s in enumerate(bs.dept_summary(rows))}
    out = [b for b in buckets.values() if b["noCard"] or b["late"]]
    out.sort(key=lambda b: order.get(b["dept"], len(order)))
    for b in out:
...
```
Expected: PASS（8 例）。注意此时整个测试文件仍会因 `render_card` / `build_text_payload` 尚未定义而 import 失败——若出现 `ImportError`，把这两个名字**暂时从测试 import 行移除**，Task 2 完成后加回。
