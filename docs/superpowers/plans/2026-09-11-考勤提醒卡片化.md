...
- Produces: `build_feishu_payload(card) -> {"msg_type": "interactive", "card": card}`；`build_text_payload(text) -> {"msg_type": "text", "content": {"text": text}}`
...
def build_feishu_payload(card):
    ...
    字段名说明（高置信度但未实测）：群自定义机器人 webhook 的 interactive 消息把卡片对象
    放在顶层 `card` 键；`content` 是 `im/v1/messages` OpenAPI 的写法。实发时按 Task 4 Step 1
    的 runbook 裁决字段名（参数类错误码即改用 `content` 重发最小卡片）。
    """
...
> **本任务已于 2026-09-15 在 Owner 授权下执行完毕。** 结果记录在
> `docs/HBOS考勤判定规则.md` §13.9 与 spec §7.1；关键裁决：**顶层 `card` 键渲染成功（`code: 0`），
> `content` 被拒（`code: 19002`）** —— 原稿写的 `content` 会导致卡片每天静默消失。
> 下方步骤保留作为当时的执行记录，不需要重跑。
...
卡片组件在飞书群机器人 webhook 上的兼容性，**以及 interactive 报文的字段名**，都无法离线验证，
故先发一张最小卡片。**字段名存疑**（spec §6.3）：高置信度认定群机器人 webhook 用顶层 `card` 键
（`content` 是 `im/v1/messages` OpenAPI 的写法），但未实测；本步即裁决它。验证必须**程序化**，
不得临场发挥——下面这段先试 `card`，失败即自动改用 `content` 重发**同一张**最小卡片。
...
import os
from hb_attendance_app.hbos_attendance.attendance_notify import post_to_webhook
...
url = os.environ.get("HBOS_NOTIFY_WEBHOOK_URL", "")
ok, msg = post_to_webhook(url, "", {"msg_type": "interactive", "card": MIN})
print("A card   :", ok, msg)
if not ok:      # 参数类错误码即换 content 重发同一张最小卡片
    print("B content:", post_to_webhook(url, "", {"msg_type": "interactive", "content": MIN}))
PY
```

判定规则（按顺序执行，不得跳步）：

1. A（顶层 `card`）返回 `(True, 'HTTP 200')` 且群里出现橙边卡片 → **字段名定为 `card`**，停。
2. A 失败（尤其 `code` 指向参数 / 字段错误）→ 看 B（顶层 `content`）；B 成功 →
   **字段名定为 `content`**，并同步把 `build_feishu_payload` 改成用 `content`，停。
3. **把实际胜出那一行的返回 `code` 原样记下**（写入本轮报告与 `docs/HBOS考勤判定规则.md` 的待验证说明），
   供事后核对。
4. A、B 都失败 → 再试把 `header.title.tag` 与文本元素 `tag` 降级为卡片 v1 结构重试。
5. 仍失败 → 执行保底：保留纯文本路线（`build_text_payload`），向 Owner 说明卡片路线不可行、停止后续步骤。

> 「参数错误即换 `content` 重发」是核心：字段名判错时飞书只回一个非零 `code`，
> 而按 §7 卡片被拒**不会**自动补发纯文本——不试第二遍，提醒会每天静默消失。
