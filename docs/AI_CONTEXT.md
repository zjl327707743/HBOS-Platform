...
当前阶段：M0 工程启动与上下文治理已完成并封板；M1 产品交付仍在 M1-FIX 功能补漏中，尚未完成。M1-FIX-B2 已 COMPLETED；M1-FIX-B3 为 REVIEWING / Owner UI 验收未通过；M1-FIX-B4 为 REVIEWING / Claude PASS，但 Owner 数据链路验收发现后续问题；M1-FIX-B5 为 REVIEWING；当前 **M1-FIX-F（调休模块第一阶段）为 REVIEWING / 已上线并通过验收**。

当前目标：M1-FIX-F 已上线，等待 Owner 判读核实结果并决定是否进入第二阶段。M1-FIX-B3 / B4 / B5 不 closeout。M1 产品交付仍未完成，M1-FIX-C/D/E 未启动。M2 未启动 / 待 Owner 授权。
...
**M1-FIX-F 调休模块第一阶段（2026-09-22）**：调休按「模板同请假、判定独立」实现——调休日取飞书日期字段区间；加班日由 LLM 从「说明」自由文本提取；核实加班日当天是否有完整上下班配对，全部通过才「已核实」。交付 `hbos_attendance/rest_leave.py`（纯标准库模块）、DocType `HBOS Rest Leave Record` 的 4 个新字段、`sync_rest_leave.py` 三段（同步 → LLM 解析 → 核实，同一 `*/30` 有序列表），并退休从未 migrate 的换班全部产物。分支 `m1-fix-c-rest-leave`（16 提交），全量测试 311 → **396 通过**。**关键约束：LLM 结果落库，判定热路径永不调用 LLM**（考勤每 10 分钟重算）。**本阶段只出核实结论供人工复核，不改变任何考勤结果**；接入判定是第二阶段，且必须先处理审批状态过滤、重新解析失效键含 employee、终态可重核、调度频率四项前提。2026-09-22 已上线：119 条入库、103 条解析、核实 40 已核实/53 不通过/14 解析失败；考勤结果与上线前逐值一致（零副作用）。上线中修复三项阻断（模型下线、HBOS_AI_* 未注入队列容器、nginx 需 reload），并在 HBOS考勤判定规则.md 就地更正「restart 保持容器 IP」的错误结论。主文档 `docs/milestones/M1_FIX_F_调休模块第一阶段落地记录.md`。命名说明：原拟用 `M1-FIX-C`，因该编号已属「异常说明三级流程」，改用 `M1-FIX-F`，待 Owner 确认。
...
2. M1 历史 closeout 已完成，但产品交付仍在 M1-FIX 中，尚未完成。M1-FIX-B3 为 REVIEWING / Owner UI 验收未通过；M1-FIX-B4 为 REVIEWING / Claude PASS，但 Owner 数据链路验收发现后续问题；M1-FIX-B5 为 REVIEWING；**M1-FIX-F 为 REVIEWING / 已上线并通过验收**。M2 未启动 / 待 Owner 授权。
