...
当前目标：等待用户决定是否进入 M1-R4：Demo 技术方案与实施路线拆分。M1-REQ-DESIGN-DRAFT 已通过 Codex 审查（初审员工姓名脱敏 blocker 已修复，复审 PASS），四份设计文档全部完成收口。M1-R4 仍为 PLANNED，尚未启动。当前仍不建议创建 `hb_hr_app`。
