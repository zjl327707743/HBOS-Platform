...
3. M3 仓储库存数字化管理已由 Owner 授权新开，与 M1-FIX 并列。M3-R0、M3-R1、M3-R2 均为 REVIEWING（M3-R2 已执行完毕）；M3-R3 至 M3-R7 为 PLANNED / 待 Owner 逐轮授权。M3 优先复用 ERPNext 原生 `Stock` 模块，不重写已有功能。Owner 已授权创建 `hb_inventory_app`，并已提供产品主数据口径：物料代码 8 位、SAP 按前四位分大类（1000 原料药 / 1100 包材 / 120x 辅助用品 / 1300 中间体 / 1400 成品）；六车间 8 个库位需一并建入 `Warehouse`；计量单位按实际建 11 种；盘点按库级三对账（ERP数量 / 货位卡数量 / 实物数量）。
