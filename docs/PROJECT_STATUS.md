...
- 下一步路线：M1-FIX-C（异常三级流程）为 PLANNED / 待 Owner 授权；不自动启动 M1-FIX-C/D/E。M3-R6（入库拍照识别）方案已就绪，PLANNED / 待 Owner 确认门禁并授权。M2 未启动。
...
本轮为**方案轮**，只固化技术选型与实施路径，**未执行任何实现**，未启动任何外部服务。

方案要点：

- **门禁三项本质是同一问题**：部署位置 / 模型选型 / 数据留存与脱敏，归根到底问的是「**标签照片能不能出内网**」。这是**合规决策而非技术决策**，须 Owner 拍板。
...
- **推荐路径**：先用方案 A 做 **20–50 张样本试跑**，量出准确率后由 Owner 决定长期走 A 还是 C。理由：走 C 的代价（需 GPU 机器 + 调优周期）值不值，取决于 A 的准确率差多少——先量后决。
- **核心工程价值**：**识别后端可插拔**。A / B / C 实现同一契约，切换不需重写服务，把"数据出不出内网"变成配置问题。
- **环境现状（实测，影响方案 C）**：承载环境为**本机 Docker（非公司服务器）**；Apple **M4 / 16GB / 无 NVIDIA GPU**；bench 内无 `cv2` / `paddleocr` / `torch` / `numpy`。**方案 C 目前没有可部署的硬件**。
...
**待 Owner 确认 6 项**：① 照片是否出内网（核心）；② 若允许，选哪家服务；③ 若不允许，部署在哪台机器；④ 标签样本从哪来；⑤ 准确率验收标准；⑥ 使用范围（全产品还是原料药/中间体先试点）。

M3-R6 未做（本轮）：未启动外部 FastAPI 服务、未调用任何视觉模型、未创建任何代码与服务、未索取标签样本、未修改核心源码、未接飞书写入、未执行 `docker compose down -v`、未删除 volume、未重建 site。
