...
- 下一步路线：M1-FIX-C（异常三级流程）为 PLANNED / 待 Owner 授权；不自动启动 M1-FIX-C/D/E。M3-R6（入库拍照识别）为 IN_PROGRESS，待仓库样本做准确率实测。M2 未启动。
...
状态：IN_PROGRESS（门禁已确认；Owner 已授权；**服务骨架、约束校验层、Frappe 侧入口与校对界面均已交付并验证**；真实端到端待授权启动服务，准确率待仓库样本）。
...
M3-R6 未做（累计）：未安装 C1/C2 重型 ML 依赖、未启动常驻服务（故 **Frappe → HTTP → FastAPI 真实跨进程链路尚未实测**）、未索取标签样本、未做批量拍照、未修改核心源码、未接飞书写入、未执行 `docker compose down -v`、未删除 volume、未重建 site。
