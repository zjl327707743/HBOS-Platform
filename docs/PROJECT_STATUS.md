...
- ⚠ **入口三处缺陷更正（2026-09-23，Owner 验收发现）**：Owner 报「点进去会跑到库存模块，桌面又找不到仓储库存工作台、回不来」，核查属实 —— ① 顶层图标用了 `stock`，**与 ERPNext 原生 Stock 模块图标完全相同**，点错即进原生模块而那边没有本 App 侧边栏；② **工作台本身没有桌面条目**（考勤那边有 `海滨考勤工作台` 挂在入口下，本 App 漏做）；③ 入口名 `仓储库存` ≠ 工作台名。**根因**：考勤有 `tests/test_workspace_contract.py` 锁住入口契约，本 App 当时没建测试。**已修**：图标改 `warehouse`、新增 `_sync_workspace_desktop_icon()`、补 7 条契约测试。库内实测与测试均通过；**未在真实浏览器验证**。详见 M3-R3 主文档 6.2 节。
