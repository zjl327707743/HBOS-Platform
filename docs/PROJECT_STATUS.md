...
- M1-R3B 已完成运行态最小修复方案，状态为 REVIEWING；本轮未执行修复、未清理 TEST 数据、未继续试运行。
...
7. M1-R3B：REVIEWING，运行态最小修复方案；本轮只写方案，未执行修复。
8. M1-R3C：PLANNED，HRMS 原生考勤重新试运行，尚未启动。
9. M1-R4：PLANNED，尚未启动。
...
- M1-R3B 只制定运行态最小修复方案，状态为 REVIEWING。
...
- 方案保留当前 TEST 数据范围：Employee 8、Shift Type 4、Shift Assignment 6、Employee Checkin 12，Attendance 0。
...
- 未执行运行态修复。
- 未启动、重启或修复 Redis / worker / scheduler / websocket。
- 未执行 `bench doctor` 修复。
- 未清理或删除 TEST 数据。
- 未继续创建测试数据。
- 未执行 HRMS 配置试运行。
