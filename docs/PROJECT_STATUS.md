...
- M1-R3A 设为 PLANNED，用于运行态阻断诊断与 TEST 数据隔离 / 清理方案。
...
6. M1-R3A：PLANNED，运行态阻断诊断与 TEST 数据隔离 / 清理方案。
7. M1-R4：PLANNED，尚未启动。
