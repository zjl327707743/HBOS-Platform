...
**安全提示**：服务绑 `0.0.0.0` 才能被容器访问，意味着同局域网可访问 8100；临时验证风险窗口小，**正式部署须缩小暴露面**。已写入服务 README。

M3-R6 未做（累计）：未安装 C1/C2 重型 ML 依赖、未索取标签样本（准确率实测前提）、未做批量拍照、未修改核心源码、未接飞书写入、未执行 `docker compose down -v`、未删除 volume、未重建 site。
