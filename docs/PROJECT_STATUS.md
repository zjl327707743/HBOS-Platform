...
⑬ **修货位卡流水行坍成一条线**（Owner 看生成的卡指出「行距太窄」）：实测差了 5.6 倍——官方受控表流水行 `trHeight=425 twips = 7.5mm`，我们只有 **1.27～1.35mm**。根因是模板里那条 `.hbos-card td.h { height: 8mm }` **没有任何单元格挂 `class="h"`**，是死规则，于是留给手写的空行没有高度约束、塌到只剩边框加内边距。已改为 `.hbos-card tr.flow td { height: 7.5mm }` 并给两处流水行（已填 + 空白）加 `class="flow"`；**外购卡同样中招，一并修了**。实测渲染 ≈8.7mm（`td` 的 padding 加在 height 之上，比官方略宽），两张卡仍各 1 页 A4、待检证无回归；**9 份已生成 PDF 全部重出**。**遗留**：行数 12 vs 官方 16，属版式容量问题、本批未动。详见主文档**第九之二十节**。
⑭ **交付《仓库工作台操作手册》并修一处取批号的不一致**（Owner 要求「出一份操作说明，进到库存单据里看不懂哪些要填」）：新增 `docs/inventory/仓库工作台操作手册.md`，按仓库操作人员口径写——从哪进、左侧 24 条用途、一张库存单的结构与三类移动类型各要填哪几个仓库格、入库/出库/移库分步操作、提交后自动生成卡证、**常见提示对照表**（照抄系统原文报错逐条给处理）、三条纪律。**点明了三处会误导人的系统名词**：「其他入库」就是正常入库（`Material Receipt`）、「其他出库」就是正常出库、「物料移动」就是 `Stock Entry`。顺手修一处真缺陷：v16 明细行批号有两条录入路径（`item.batch_no` / `serial_and_batch_bundle`），`release_gate.py` 两条都读，而 `doc_gen._distinct_batches()` 只读第一条 —— 走弹窗那条路时**入库后不出货位卡、且不报错**；已补齐回落，两处取法一致。验证三条路径分别跑通 + 正常批次仍能出 PDF。详见主文档**第九之二十一节**。
