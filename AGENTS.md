...
当前并行里程碑为 M1-FIX（考勤功能补漏，IN_PROGRESS）与 M3（仓储库存数字化管理，IN_PROGRESS，M3-R0 / R1 / R2 / R3 / R4 / R5 已执行）。权威进度以 `docs/PROJECT_STATUS.md`、`docs/CURRENT_MILESTONE.md` 为准。
...
- 未经用户逐轮授权启动 M3-R6 及之后轮次、创建 M3 相关 DocType、编写 M3 业务代码或创建库存业务数据
...
**独立前端开发规则**：凡涉及独立前端、驾驶舱、AI 工作台、复杂交互页面，必须先原型设计、Owner 审查通过后再复刻实现。详见 `docs/frontend/FRONTEND_IMPLEMENTATION_GUIDE.md`。
